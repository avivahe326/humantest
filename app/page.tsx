    ...
    <div className="space-y-24 pb-16">
      ...
      <section className="space-y-6">
        ...
        <Card className="mx-auto max-w-2xl">
          <CardContent className="pt-6">
            <pre className="overflow-x-auto rounded bg-muted p-4 text-sm leading-relaxed"><code>{/* comment */}<span className="text-muted-foreground italic">{'// One API call. Real human feedback.'}</span>{'\n'}<span className="text-purple-400">const</span> response = <span className="text-purple-400">await</span> <span className="text-blue-400">fetch</span>(<span className="text-green-400">{`'/api/skill/human-test'`}</span>, {'{'}{'\n'}  method: <span className="text-green-400">{`'POST'`}</span>,{'\n'}  headers: {'{'}{'\n'}    <span className="text-green-400">{`'Authorization'`}</span>: <span className="text-green-400">{`'Bearer <your-api-key>'`}</span>,{'\n'}    <span className="text-green-400">{`'Content-Type'`}</span>: <span className="text-green-400">{`'application/json'`}</span>,{'\n'}  {'}'},{'  '}{'\n'}  body: <span className="text-blue-400">JSON</span>.<span className="text-blue-400">stringify</span>({'{'}{'\n'}    url: <span className="text-green-400">{`'https://your-product.com'`}</span>,{'\n'}    focus: <span className="text-green-400">{`'Test the onboarding flow'`}</span>,{'\n'}    maxTesters: <span className="text-orange-400">5</span>,{'\n'}  {'}'}),{'\n'}{'}'});{'\n'}{'\n'}<span className="text-purple-400">const</span> {'{ '}<span className="text-foreground">taskId</span>{' }'} = <span className="text-purple-400">await</span> response.<span className="text-blue-400">json</span>();{'\n'}<span className="text-muted-foreground italic">{'// Testers will test your product and submit feedback.'}</span>{'\n'}<span className="text-muted-foreground italic">{"// You'll receive a webhook with the full AI report."}</span></code></pre>
          </CardContent>
        </Card>
      </section>
