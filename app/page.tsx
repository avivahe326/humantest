      ...
      <section className="space-y-6">
        ...
        <Card className="mx-auto max-w-2xl">
          <CardContent className="pt-6">
            <pre className="overflow-x-auto rounded bg-muted p-4 text-sm leading-relaxed"><code><span className="text-muted-foreground italic">{'// One API call. Real human feedback.'}</span>{'\n'}<span className="text-purple-400">const</span> {'{ taskId }'} = <span className="text-purple-400">await</span> <span className="text-blue-400">fetch</span>(<span className="text-green-400">{`'/api/skill/human-test'`}</span>, {'{'}{'\n'}  method: <span className="text-green-400">{`'POST'`}</span>,{'\n'}  headers: {'{ '}<span className="text-green-400">{`'Content-Type'`}</span>: <span className="text-green-400">{`'application/json'`}</span>{' },'}{'\n'}  body: <span className="text-blue-400">JSON</span>.<span className="text-blue-400">stringify</span>({'{'}{'\n'}    url: <span className="text-green-400">{`'https://your-product.com'`}</span>,{'\n'}    focus: <span className="text-green-400">{`'Test the onboarding flow'`}</span>,{'\n'}    maxTesters: <span className="text-orange-400">5</span>,{'\n'}    repoUrl: <span className="text-green-400">{`'https://github.com/you/repo'`}</span>,  <span className="text-muted-foreground italic">{'// get code fix suggestions'}</span>{'\n'}    webhookUrl: <span className="text-green-400">{`'https://your-server.com/webhook'`}</span>,  <span className="text-muted-foreground italic">{'// report ready notification'}</span>{'\n'}    codeFixWebhookUrl: <span className="text-green-400">{`'https://your-server.com/code-fix'`}</span>,  <span className="text-muted-foreground italic">{'// code fix ready notification'}</span>{'\n'}  {'}'}),{'\n'}{'}'}).<span className="text-blue-400">then</span>(r {'=> '}r.<span className="text-blue-400">json</span>());{'\n'}{'\n'}<span className="text-muted-foreground italic">{'// Option 1: Poll for results'}</span>{'\n'}<span className="text-purple-400">const</span> result = <span className="text-purple-400">await</span> <span className="text-blue-400">fetch</span>(<span className="text-green-400">{`\`/api/skill/status/\${taskId}\``}</span>).<span className="text-blue-400">then</span>(r {'=> '}r.<span className="text-blue-400">json</span>());{'\n'}result.report  <span className="text-muted-foreground italic">{'// AI-generated usability report'}</span>{'\n'}{'\n'}<span className="text-muted-foreground italic">{'// Option 2: Receive webhook POST when report is ready'}</span>{'\n'}<span className="text-muted-foreground italic">{'// → { taskId, status, title, targetUrl, report, completedAt }'}</span></code></pre>
          </CardContent>
        </Card>
      </section>
